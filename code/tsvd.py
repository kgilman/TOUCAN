...
def normalizeTensorVec(v):
    ...
    # vnormal = Tensor(np.real(np.fft.ifft(vnormal_F, axis=2)))
    # vnorms = Tensor(np.real(np.fft.ifft(vnorms_F, axis=2)))
...
def tctf(Y,mask,rank,niter=100,min_iter = 10,tol=1e-8,it_tol = 1e-4,fun=lambda U,V: [0, 0],verbose=False):
    ...
    for iter in range(0, niter):
        ...
        # it_diff = tfrobnorm(C - C0)
        # C0 = Tensor(C.array().copy())
...
def trpca(T, mask, beta, niter, rho=1.1, mu=1e-4, fun=lambda L, S, Y, mu: [0, 0, 0], verbose=False):
    ...
    for iter in range(0, niter):
        ...
        # X = mask*T.array() + (mask < 1).astype(float)*(L0 + S0)
...
def toucan_stream_tube(Yvec,idx,U,step=None):
    ...
    for i in range(0, int(np.ceil((n3 + 1) / 2))):
        # step = np.arctan(rnorms_F[0,0,i] / wnorms_F[0,0,i])
        # alpha = (np.cos(step) - 1)
        # beta = np.sin(step)
        if (step is None):
...
def cg(Ubar, vbar, mask, cg_iter=None, tol=1e-12):
    ...
    for i in range(0, cg_iter):
        ...
        # if(np.real(rsnew) < tol):
        # if(np.linalg.norm(wbar - wbar_last) < tol):
            break
        # else:
        #     wbar_last = wbar
...
def cg_test(Ubar, vbar, mask, cg_iter=None, tol=1e-12):
    ...
    # phi_array = np.zeros(cg_iter)

    eps = np.arctan(np.real(fourier_dot(r,r)) / np.real(fourier_dot(Aw,Aw)))

    num_iters = 0
    for i in range(0, cg_iter):

        # k = max(10,int(0.1*(i+1)))

        Ap = A(p)
        pAp = fourier_dot(p, Ap)
        alpha = rsold / pAp
        wbar = wbar + alpha * p
        r = r - alpha * Ap
        rsnew = fourier_dot(r, r)

        # phi = 0.5*pAp - fourier_dot(bbar,p)
        # phi_array[i] = phi

        # if(i + 1 > k and phi < 0):
        #     test = (phi - phi_array[i + 1 - k]) / phi
        #     if((phi - phi_array[i + 1 - k]) / phi < k*eps):
        #         break
...
def toucan_stream(Yvec, M, U, step=None, cgiter = None, cgtol = None):
    ...
    # wbar,cg_iters = cg_test(Ubar, vbar, M, tol=cgtol)
