...
def compute_stats(Rhat, Chat, R, C, orig_im, kdata_orig):
    ...
    # nrmse_test = np.sqrt(np.sum((rec_im - orig_im)**2) / np.sum(orig_im**2))
    # nrmse_test2 = np.linalg.norm(rec_im[vis_idx,:,:] - orig_im[vis_idx,:,:])
    # nrmse_test3 = 0
    # for i in range(0,70):
    #     nrmse_test3 += np.linalg.norm(rec_im[i,:,:] - orig_im[i,:,:],'fro') / np.linalg.norm(orig_im[i,:,:],'fro')
    #
    # nrmse_test3 = np.sqrt(nrmse_test3)
